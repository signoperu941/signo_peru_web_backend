...
async def buscar_palabra(q: str):
    ...
    if ruta:
        ...
        return {"palabra": q, "encontrada": True, "url_streaming": ruta}
    else:
        print(f"[busqueda] No encontrada: '{q}'")
        return {"palabra": q, "encontrada": False, "url_streaming": None}
                            ...
                            palabras.append(
                                {
                                    ...
                                    "tiene_video": bool(item.get("ruta_local")),
                                }
                            )
                            total_palabras += 1
                    if palabras:
...
async def obtener_datos_completos_learn():
    ...
    print(
        f"[busqueda] Learn-data solicitado | Categorías: {total_categorias} | Palabras: {total_palabras}"
    )
    return {
